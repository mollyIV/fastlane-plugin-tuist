...
module Fastlane
  module Actions
    class TuistScaffoldListAction < Action
      def self.run(params)
        command = ['list']
      ...
      def self.available_options
        [
          FastlaneCore::ConfigItem.new(
            key: :path,
            description: "The path where you want to list templates from (env: TUIST_SCAFFOLD_LIST_PATH)",
            optional: true,
            type: String
          ),
          ...
          FastlaneCore::ConfigItem.new(
            key: :json,
            description: "The output in JSON format (env: TUIST_SCAFFOLD_LIST_JSON)",
            optional: true,
            type: Boolean
          ),
